...
const EditarPerfil = () => {
  ...
  // Estados para editar la pregunta de recuperación
  ...
  useEffect(() => {
    ...
    const fetchPerfil = async () => {
      try {
        ...
        // Si ya tiene asignada una pregunta, se establece el ID para el dropdown
  ...
  const handleSubmit = async (e) => {
    ...
    // Si se ingresa nueva contraseña, incluirla en la actualización
    if (newPassword) {
      updateData.password = newPassword;
    }

    // Si se selecciona una nueva pregunta y se ingresa respuesta, actualizar la recuperación
  ...
  return (
    // Contenedor con fondo degradado y altura mínima de 100vh
    <div
      style={{
